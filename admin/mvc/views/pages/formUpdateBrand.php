  ...
  <main class="app-content">
    <div class="app-title">
      <ul class="app-breadcrumb breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="#">Sửa thông tin nhà cung cấp</a></li>
      </ul>
    </div>
        <div class="tile">
          <h3 class="tile-title">Sửa thông tin nhà cung cấp</h3>
          <div class="tile-body">
          <div class="row">
        <div class="col-md-6 offset-md-3">
            <!-- Form nhập tên danh mục -->
            <form action="brandUpdateController/update_brand" method="post">
                ...
                <?php
                    if($row2 = mysqli_fetch_array($data["getbrand"])){
                ?>
                <div class="form-group">
                    <label class="control-label">Tên nhà cung cấp:</label>
                    <input type="text" class="form-control" id="brandName" name="brandName" value="<?php echo $row2["brand_name"]; ?>">
                    <input type="hidden" class="form-control" id="brandID" name="brand_id" value="<?php echo $row2["brand_id"]; ?>">
                    <input type="hidden" class="form-control" id="category_id" name="category_id" value="<?php echo $row["cartegory_id"]; ?>">
                </div>
                <?php
                    }
                ?>
                <button type="submit" class="btn btn-primary" name="btnUpdateBrand">Sửa</button>
            </form>
        </div>
    </div>
        </div>
